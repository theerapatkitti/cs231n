...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################    
    scores -= np.max(scores, axis=1, keepdims=True) #shift value of scores to prevent numeric instability
    exp_scores = np.exp(scores)
    
    loss = np.sum(-np.log(exp_scores[np.arange(N), y] / np.sum(exp_scores, axis=1)))
