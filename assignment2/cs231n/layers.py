...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (xhat, xmu, sqrtvar, ivar, gamma)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, D = dout.shape
    xhat, xmu, sqrtvar, ivar, gamma = cache
    ...
    # differentiate summation
    dsq = dvar * 1.0 / N * np.ones((N, D))
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        keep_prob = 1 - p
        mask = (np.random.rand(x.shape[0], x.shape[1]) < keep_prob) / keep_prob # use inverted dropout
        out = x * mask
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    n_H = int(1 + (H + 2 * pad - HH) / stride)
    n_W = int(1 + (W + 2 * pad - WW) / stride)
    n_C = F
    
    out = np.zeros((N, n_C, n_H, n_W))
        ...
        for h_ in range(n_H):
            for w_ in range(n_W):
                for c in range(n_C):
                    ...
                    dx_pad[i, :, vert_start:vert_end, horiz_start:horiz_end] += w[c] * dout[i, c, h_, w_]
                    dw[c] += x_slice * dout[i, c, h_, w_]
                    db[c] += dout[i, c, h_, w_]
...
def max_pool_forward_naive(x, pool_param):
    ...
    n_H = int(1 + (H - pool_height) / stride)
    n_W = int(1 + (W - pool_width) / stride)
    n_C = C
        ...
        for h in range(n_H):
            for w in range(n_W):
                for c in range(n_C):
                    ...
                    dx[i, c, vert_start:vert_end, horiz_start:horiz_end] += mask * dout[i, c, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
###########################################################################
# ADDITIONAL FEATURE: Average pooling layer                               #
###########################################################################
def avg_pool_forward_naive(x, pool_param):
    """
    A naive implementation of the forward pass for an average pooling layer.
        ...
        for h in range(n_H):
            for w in range(n_W):
                for c in range(n_C):
                    ...
                    out[i, c, h, w] = np.average(x_slice)
...
def avg_pool_backward_naive(dout, cache):
    """
    A naive implementation of the backward pass for an average pooling layer.
        ...
        for h in range(n_H):
            for w in range(n_W):
                for c in range(n_C):
                    ...
                    average = np.float(dout[i, c, h, w]) / np.float(pool_height * pool_width)
                    
                    dx[i, c, vert_start:vert_end, horiz_start:horiz_end] += np.ones((pool_height, pool_width)) * average
